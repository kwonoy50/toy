DROP TABLE IF EXISTS _G_BOARD;
CREATE TABLE _G_BOARD (
    BNO     VARCHAR(4)    NOT NULL,
    TITLE   VARCHAR(200)  NOT NULL,
    CONTENT VARCHAR(4000) NOT NULL,
    WRITER  VARCHAR(50)   NOT NULL,
    REGDATE VARCHAR(8)   NOT NULL,
    CONSTRAINT PK_G_BOARD UNIQUE (BNO)
);
COMMENT ON TABLE  _G_BOARD         IS '게시물';
COMMENT ON COLUMN _G_BOARD.BNO     IS '번호';
COMMENT ON COLUMN _G_BOARD.TITLE   IS '제목';
COMMENT ON COLUMN _G_BOARD.CONTENT IS '내용';
COMMENT ON COLUMN _G_BOARD.WRITER  IS '작성자';
COMMENT ON COLUMN _G_BOARD.REGDATE IS '작성일';
